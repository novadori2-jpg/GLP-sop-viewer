...
function toStudyInfo(row: Record<string, unknown>) {
  return {
    ...
    formulatorId: row.formulator_id ?? undefined,
    formulatorName: row.formulator_name ?? undefined,
    qapId: row.qap_id ?? "",
    qaName: row.qa_name ?? "",
...
export async function POST(req: NextRequest) {
  ...
  const { error } = await supabase.from("binders").insert({
    ...
    formulator_id: body.formulatorId ?? null,
    formulator_name: body.formulatorName ?? null,
